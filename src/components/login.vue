                      ...
                      <b-row>
                        ...
                        <b-row>
                          <b-col  cols="4" class=" mb-4">
                            <b-button variant="link" class="mb-3" ><router-link to="/signup">Sign Up Here</router-link></b-button>
                          </b-col>
                        </b-row>
                      </b-row>
                    </form>
...
<script>
import { signIn } from "../../cognito-services/index.js";
export default {
  name: "Login",
  beforeCreate() {
          ...
          signIn(email, password)
            .then(data => {
              ...
              if (data.userStatus === "CONFIRMED") {
                ...
                this.$store.commit("setAuthStorageTokens", {
                  authStorageTokens: data.authStorageTokens
                });
                this.$router.push("/");
              } else if (data.userStatus === "FORCE_CHANGE_PASSWORD") {
                // Wait and do nothing
              } else {
                this.$toasted.show("Not authenticated/confirmed user, ", data);
              }
            })
            .catch(err => {
              console.log("Err:", err);
